...
\begin{figure}[h]
    ...
    \end{tikzpicture}
\end{figure}
追加使用に関しては，\ref{sec:仕様3処理}節，XX節で詳説する．
\newpage
\subsection{処理}\label{sec:仕様3処理}
\subsubsection*{\met{openAllTiles}}
全てのタイルに対して，条件分岐を行う．
\begin{enumerate}
    \item そのタイルに旗は立っていたが，爆弾ではない場合，タイルに"\verb|XB|"と表示する．\label{enu:XB}
          \begin{lstlisting}[caption=, label=, language=Java]
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        ...
        \foreach \u in {decision1,decision2}
        \draw[-latex](\u.east)-|(return2)--(return.east);
        % \coordinate (C) at (right=1cm of decision2);
        % \node (D) at (right=1cm of decision1 |- decision2.east){};
    \end{tikzpicture}
\end{figure}
\newpage
\subsection{処理}
...
\begin{enumerate}
    ...
    \item \verb|openTile|メソッドに，タイルにフラグがあれば\verb|return;|する処理がある．
          \begin{lstlisting}[caption=, label=, language=Java]
