...
\vspace{-0.5cm}
\subsection*{周りの爆弾の数}
このレポートでいう「周りの爆弾の数」とは，あるタイル\verb|A,B|に対して，\figref{fig:周りの爆弾}の\ \begin{tikzpicture}[remember picture,baseline=(putcolor.base)]
...
\begin{figure}[h]
    ...
    \begin{minipage}[t]{0.45\linewidth}
        ...
        \begin{tikzpicture}[remember picture]
            ...
            \draw[help lines, step=1cm](0,0) grid (3,3);
            \coordinate (gri) at (3.3,2.5);
            % \draw[fill=black](gri)circle[radius=0.06];
        \end{tikzpicture}
    \end{minipage}
\end{figure}
\begin{tikzpicture}[remember picture,overlay]
    \draw[->,dashed,thick](putcolor.south east)to[bend left=45](gri);
\end{tikzpicture}
\newpage
\part{必須仕様}
\section*{仕様1}
